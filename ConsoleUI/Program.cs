...
namespace ConsoleUI
{
    ...
    {
	    List<EnterModel> timeSheets = LoadTime.LoadTimeSheets();
	    List<CustomerModel> customers = Data.GetCustomers();
	    EmployeeModel currentEmployee = Data.GetCurrentEmployee();

	    customers.ForEach(x => BillCustom.BillCustomer(timeSheets, x));

	    PayEmp.PayEmployee(timeSheets, currentEmployee);
